...
server.post("/signup", (req, res) => {
  ...
  if (!passwordRegex.test(password)) {
    return res.status(400).json({
      message:
        "Password must be 6-20 characters, contain at least one digit, one lowercase, and one uppercase letter",
    });
  }
...
server.post("/latest-blogs", (req, res) => {
  ...
  Blog.find({ draft: false })
    ...
    .select("blog_id title banner des tags publishedAt author activity -_id")
    .populate(
      "author",
...
server.get("/trending-blogs", (req, res) => {
  Blog.find({ draft: false })
    ...
    .limit(5)
    .select("blog_id title publishedAt author activity -_id")
    .populate(
      "author",
      "personal_info.fullname personal_info.username personal_info.profile_img -_id"
    )
  ...
  try {
    for (let i = 0; i < social_links_keys.length; i++) {
      if (social_links[social_links_keys[i]].length) {
        ...
        if (
          !hostname.includes(`${social_links_keys[i]}.com`) &&
          social_links_keys[i] !== "website"
        ) {
          return res.status(400).json({
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (id) {
    Blog.findOneAndUpdate(
      { blog_id },
      { title, banner, content, tags, des, draft: Boolean(draft) }
    )
      .then(() => {
        return res.status(200).json({
          id: blog_id,
    ...
    if (replying_to) {
      ...
      await Comment.findOneAndUpdate(
        ...
        .then((replyingToCommentDoc) => {
          commentNotification.notification_for =
...
server.post("/notifications", verifyJWT, (req, res) => {
  ...
  Notification.find(findQuery)
    ...
    .then((notifications) => {
      Notification.updateMany(findQuery, { seen: true }).then(() => {
...
server.post("/delete-blog", verifyJWT, (req, res) => {
  ...
  Blog.findOneAndDelete({ blog_id })
    .then((blog) => {
      ...
      User.findOneAndUpdate(
        { _id: user_id },
        { $inc: { "account_info.total_posts": -1 }, $pull: { blogs: blog._id } }
      ).then(() => {
